...
## Scene selection and complete workspace design

Scene selection now ranks catalogue candidates by reported cloud cover then date offset, and round-robins acquisition days to avoid spending the entire budget on one day's overlapping tiles. Up to ten candidates are attempted within a 90-second soft selection budget (an in-flight read can exceed it; the existing job deadline still applies). The requested date window and minimum 50% usable AOI coverage are unchanged. Errors distinguish no catalogue scenes, rejected coverage and read failures, and report actual checked counts and best observed coverage. The UI avoids repeating identical error text and offers an explicit ±30-day setting; users must review and submit again. Provider/date availability is not guaranteed.

The supplied ZIP's design tokens are now the shared foundation for all live pages. Common workspace headers, source chips, parameter panels, results, tables, forms, spectral panels, recovery messages and responsive layouts cover Ask Satellite, Compare Changes, Optical & Radar, Crop Outlook, research and data sources. Mock authentication and fabricated content remain excluded. Tests: 47 backend, 31 frontend, lint and build pass. Visual browser validation remains outstanding because the available browser cannot reach local previews. No exact failed-region reproduction was possible without the user's coordinates.
